...
- Use reusable Figma script templates for phone frames, component kit seeds, movable bitmap crops, and layer-audit metadata when reconstructing generated images into Figma.
- Run a visual difference check before claiming 1:1: compare source image and Figma screenshot for size, crop, color, hierarchy, spacing, icons, chrome, and layer editability.
- Follow image crop specs for hero, canvas, thumbnail, avatar, product, food, generated-result, and background regions. Preserve source assets as independent movable crops instead of redrawing or replacing them.
...
- For Figma reconstruction scripts and reusable layer/component templates, read `references/workflow/figma-script-templates.md`.
- For source-image versus Figma screenshot comparison, read `references/workflow/visual-diff-check.md`.
