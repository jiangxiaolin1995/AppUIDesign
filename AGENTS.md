...
仓库根目录即 Skill 根目录，`SKILL.md` 必须保持在根上。参考资料按研究、行业、流程三类拆分，避免单层目录过载。大厂规范只作为尺寸、密度、组件一致性、图片裁切和平台约束的判断来源，不作为视觉风格照搬对象。设计图、高保真和 Figma 输出前必须做 UI Notes Pin + 美叶灵感集的模式提炼。Figma 输出默认要求 1:1 且可编辑：整屏图只能做参考，最终稿必须由可移动素材、独立 bitmap crop 和组件组成，并通过视觉差异检查。
