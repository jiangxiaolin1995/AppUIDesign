...
Figma 输出默认要求可编辑且尽量 1:1：整屏图只能做参考，最终稿必须由可移动素材、独立 bitmap crop、矢量图标、文本层和组件组成，并通过视觉差异检查。不能用圆点代替正式图标。生成图片时就要同步规划 Figma 分层、图标来源、图片裁切、组件边界、状态矩阵和交付包。若用户需要间距/尺寸标注，measurement JSON 必须作为 Figma 坐标合同使用：`elements[].rect` 负责放置图层，`annotations[]` 负责画标注线；标注层和 JSON 作为独立 handoff frame/spec panel 加进 Figma，不能画进干净的可编辑 UI 里。
